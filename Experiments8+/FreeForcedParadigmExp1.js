...
// NEWIAN: changed SONA Link
const jsPsych = initJsPsych({
    on_finish: function () {
        if (PRMS.count_block >= 12) {
            window.location.assign(
                "https://uni-tuebingen.sona-systems.com/webstudy_credit.aspx?experiment_id=523&credit_token=0972700982f7404aa915b5b3b6fdf0ed&survey_code=" +
...
const PRMS = {
    ...
    n_blocks: 12,
    n_blocks_practice: 2,
    n_trials: 60, // multiple of 4
    vas_trial_interval: [100, 100], // number of trials between two vas trials //NEWIAN: changed to large number so that no VAS appears
    ...
    feedback_duration_experiment: [500, 2000, 2000], // duration of the feedback experiment (NB. no feedback when VAS trial)
...
Versuche so schnell wie möglich zu sein ohne zuviele Fehler zu machen!<br>`,
            ...
            RESPONSE_MAPPING +
            (PRMS.count_block > 100 // NEWIAN: I changed this to a large number so that the text never appears. (previous: PRMS.n_blocks_practice)
                ? generate_formatted_html({
