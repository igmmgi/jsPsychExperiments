    ...
    on_finish: function () {
        if (PRMS.cBlk >= 14) {
            window.location.assign(
                "https://uni-tuebingen.sona-systems.com/webstudy_credit.aspx?experiment_id=763&credit_token=37b93d3a3ffe40ffb92a4c4bcd6bb877&survey_code=" +
...
const WELCOME_INSTRUCTIONS = {
    ...
    stimulus: generate_formatted_html({
        text: `Willkommen zu unserem Experiment:<br><br>
               ...
               nina.fechler@student.uni-tuebingen.de<br><br>
...
const FLANKER = {
    ...
    on_start: function (trial) {
        ...
        trial.stimulus = function (c) {
            draw_stimulus(c, { colours_target: dot_colours_target });
        };
    },
    on_finish: function () {
        codeTrial();
...
const TRIAL_FEEDBACK = {
    ...
    on_start: function (trial) {
        ...
        trial.trial_duration = PRMS.fbDur[dat.displayed_corr_code - 1];

        trial.stimulus = generate_formatted_html({
            text: `${PRMS.fbText[dat.displayed_corr_code - 1]}`,
            align: "center",
            fontsize: 30,
            width: "1200px",
            lineheight: 1.5,
...
function codeTrial() {
    ...
    // True vs False feedback logic
    let is_true_feedback = true;
    if (corrCode === 1) {
        is_true_feedback = feedback_pool_correct.pop();
    } else if (corrCode === 2) {
    ...
    jsPsych.data.addDataToLastTrial({
        ...
        trialNum: PRMS.cTrl,
        corr_code: corrCode,
        displayed_corr_code: displayed_corr_code,
        is_true_feedback: is_true_feedback,
...
// prettier-ignore
const TRIAL_TABLE = [
    { target: PRMS.target[0], correct_key: PRMS.respKeys[PRMS.target.indexOf(PRMS.target[0])] },
    { target: PRMS.target[1], correct_key: PRMS.respKeys[PRMS.target.indexOf(PRMS.target[1])] },
];
...
let feedback_pool_correct = shuffle(repeat_array([true], n_true).concat(repeat_array([false], n_false)));
let feedback_pool_error = shuffle(repeat_array([true], n_true).concat(repeat_array([false], n_false)));
