...
function code_trial() {
    ...
    let corr_code = 0;
    let rt = dat.rt === null ? PRMS.too_slow : dat.rt - dat.delay;

    let comp = dat.stim1.includes(dat.stim2) || dat.stim2.includes(dat.stim1) ? "comp" : "incomp";
...
const BLOCK_FEEDBACK = {
    ...
    on_start: function (trial) {
        let block_dvs = calculate_block_performance({
            filter_options: { stim: "flanker_delay", block_num: PRMS.cblk },
        });
...
function save() {
    ...
    save_data_server("/Common8+/write_data.php", data_fn, { stim: "flanker_delay" });
    // save_data_local(data_fn, { stim: "flanker_delay" });
}
