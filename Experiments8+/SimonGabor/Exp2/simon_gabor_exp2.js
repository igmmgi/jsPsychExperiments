...
const RESP_MAPPING = VERSION === 1 ? ["Horizontal", "Vertikal"] : ["Vertikal", "Horizontal"];
...
const PRMS = {
    ...
    gabor_sf: 0.055, // Spatial frequency (cycles per pixel)
    gabor_phase: 0, // Phase offset (0-360 degrees)
    gabor_tilt: [0, 90], // Tilt/rotation in degrees (0 = horizontal, 90 = vertical)
...
const RESP_TEXT =
    generate_formatted_html({
        text: RESP_MAPPING[0] + "&emsp;".repeat(8) + RESP_MAPPING[1] + "&emsp;".repeat(8),
        bold: true,
        fontsize: 26,
        lineheight: 0.5,
    }) +
...
const TASK_INSTRUCTIONS2 = {
    ...
    stimulus:
        generate_formatted_html({
            text: `Aufgabe:<br><br>
                  In diesem Experiment musst du entscheiden, ob die Linien in einem Kreis horizontal oder vertikal orientiert sind.<br><br>
                  Der Kreis erscheint entweder links oder rechts auf dem Bildschirm.<br><br>
                  Ignoriere die Position des Kreises und reagiere so schnell und genau wie möglich auf die Orientierung der Linien wie folgt:<br><br>
...
function draw_trial_feedback(c, args) {
    ...
    if (args.corr_code !== 1) {
        let row1 = ` ${RESP_MAPPING[0]}         ${RESP_MAPPING[1]}`;
...
function save() {
    jsPsych.data.addProperties({ vpNum: VP_NUM, version: VERSION });
...
const GABOR_STIMULUS_HORIZ_LEFT_POS = {
    obj_type: "gabor",
    origin_center: true,
...
// Gabor stimulus definition (references PRMS for easy testing)
const GABOR_STIMULUS_HORIZ_RIGHT_POS = {
    obj_type: "gabor",
    origin_center: true,
    startX: PRMS.simon_pos,
    startY: 0,
...
// Gabor stimulus definition (references PRMS for easy testing)
const GABOR_STIMULUS_VERT_LEFT_POS = {
    obj_type: "gabor",
    origin_center: true,
    startX: -PRMS.simon_pos,
    startY: 0,
...
// Gabor stimulus definition (references PRMS for easy testing)
const GABOR_STIMULUS_VERT_RIGHT_POS = {
    obj_type: "gabor",
    origin_center: true,
    startX: PRMS.simon_pos,
    startY: 0,
...
const TRIAL_LOW_NOISE_HORIZ_LEFT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_LOW, FIX1, FIX2, GABOR_STIMULUS_HORIZ_LEFT_POS],
    response_type: "key",
    ...
    data: {
        ...
        comp: VERSION === 1 ? "comp" : "incomp",
        corr_resp: VERSION === 1 ? PRMS.resp_keys[0] : PRMS.resp_keys[1],
    },
    on_finish: function () {
        code_trial();
        PRMS.ctrl += 1;
...
const TRIAL_LOW_NOISE_HORIZ_RIGHT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_LOW, FIX1, FIX2, GABOR_STIMULUS_HORIZ_RIGHT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
const TRIAL_LOW_NOISE_VERT_LEFT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_LOW, FIX1, FIX2, GABOR_STIMULUS_VERT_LEFT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
    data: {
        ...
        position: -PRMS.simon_pos,
        comp: VERSION === 1 ? "comp" : "incomp",
        corr_resp: VERSION === 1 ? PRMS.resp_keys[1] : PRMS.resp_keys[0],
    },
    on_finish: function () {
        code_trial();
        PRMS.ctrl += 1;
...
const TRIAL_LOW_NOISE_VERT_RIGHT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_LOW, FIX1, FIX2, GABOR_STIMULUS_VERT_RIGHT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
const TRIAL_HIGH_NOISE_HORIZ_LEFT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_HIGH, FIX1, FIX2, GABOR_STIMULUS_HORIZ_LEFT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
const TRIAL_HIGH_NOISE_HORIZ_RIGHT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_HIGH, FIX1, FIX2, GABOR_STIMULUS_HORIZ_RIGHT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
const TRIAL_HIGH_NOISE_VERT_LEFT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_HIGH, FIX1, FIX2, GABOR_STIMULUS_VERT_LEFT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
const TRIAL_HIGH_NOISE_VERT_RIGHT = {
    type: jsPsychPsychophysics,
    pixi: pixi_flag,
    canvas: { background_color: "transparent" },
    stimuli: [BACKGROUND_HIGH, FIX1, FIX2, GABOR_STIMULUS_VERT_RIGHT_POS],
    response_type: "key",
    choices: PRMS.resp_keys,
    response_ends_trial: true,
    trial_duration: PRMS.too_slow,
...
function genExpSeq() {
    ...
    for (let blk = 0; blk < PRMS.nblks; blk++) {
        ...
        const trialArray = jsPsych.randomization.repeat(
            [
                TRIAL_LOW_NOISE_HORIZ_LEFT,
                TRIAL_HIGH_NOISE_HORIZ_LEFT,
                TRIAL_LOW_NOISE_HORIZ_RIGHT,
                TRIAL_HIGH_NOISE_HORIZ_RIGHT,
                TRIAL_LOW_NOISE_VERT_LEFT,
                TRIAL_HIGH_NOISE_VERT_LEFT,
                TRIAL_LOW_NOISE_VERT_RIGHT,
                TRIAL_HIGH_NOISE_VERT_RIGHT,
            ],
            PRMS.ntrls / 8,
        );
