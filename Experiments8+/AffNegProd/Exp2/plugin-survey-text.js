var jsPsychSurveyText = (function (jspsych) {
  ...
  class SurveyTextPlugin {
    ...
    trial(display_element, trial) {
        ...
        if (Array.isArray(trial.preamble)) {
          html += `<div style="display: flex; gap: 200px; position: relative;">
            <div style="flex: 1; overflow: visible; position: relative;">
              <img src="${trial.preamble[0]}" style="width: 100%; height: 100%; object-fit: cover;">
            </div>
          </div>`;
        } else {
          html += trial.preamble;
      ...
      if (Array.isArray(trial.preamble)) {
        const img = display_element.querySelector('#jspsych-survey-text-preamble img');
        img.onload = () => {
          const height = img.naturalHeight;
          const container = display_element.querySelector('#jspsych-survey-text-preamble div > div');
