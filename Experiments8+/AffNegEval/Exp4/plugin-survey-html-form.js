var jsPsychSurveyHtmlForm = (function (jspsych) {
  ...
  class SurveyHtmlFormPlugin {
    ...
    trial(display_element, trial) {
      ...
      html += trial.html;
      html += '<input type="submit" id="jspsych-survey-html-form-next" class="jspsych-btn jspsych-survey-html-form" value="' + trial.button_label + '" disabled></input>';
      html += "</form>";
      display_element.innerHTML = html;

      // Add event listener for radio buttons
      display_element.querySelector("#jspsych-survey-html-form").addEventListener("change", (event) => {
        const submitButton = display_element.querySelector("#jspsych-survey-html-form-next");
        const textInput = display_element.querySelector("#test-resp-box");
        
        if (event.target.type === 'radio') {
          if (event.target.value === 'has_idea') {
            // If "Yes" selected, enable button only if text field has content
            ...
            // If "No idea" selected, clear textarea and enable button
            textInput.value = '';
      ...
      function serializeArray(form) {
        ...
        for (var i = 0; i < form.elements.length; i++) {
          ...
          } else if (field.type === "radio" && field.checked && field.value === "no_idea") {
            // If "No idea" radio is checked, push empty string as value
            serialized.push({
              name: field.name,
              value: ""
            });
