...
const PRMS = {
    ...
    speed_difficulty: { easy: 0.06, hard: 0.12 }, // px/ms (0.06 = 60px/s ~10s, 0.12 = 120px/s ~5s)
    ball_diameter: 20,
...
const TRIAL = {
    ...
    on_start: function (trial) {
        let speed_diff = jsPsych.evaluateTimelineVariable("speed_difficulty");
        let path_diff = jsPsych.evaluateTimelineVariable("path_difficulty");
        PATH.reset(1);
...
function genExpSeq() {
    ...
    blk_type = rep_each(blk_type, PRMS.n_blocks / 4);

    const TIMELINE_MAP = {
        "easy_path_easy_speed": TRIAL_TIMELINE_EASY_PATH_EASY_SPEED,
        "easy_path_hard_speed": TRIAL_TIMELINE_EASY_PATH_HARD_SPEED,
        "hard_path_easy_speed": TRIAL_TIMELINE_HARD_PATH_EASY_SPEED,
        "hard_path_hard_speed": TRIAL_TIMELINE_HARD_PATH_HARD_SPEED
    };

    for (let blk = 0; blk < PRMS.n_blocks; blk += 1) {
        exp.push(BLOCK_START); // trials within a block
        let blk_timeline = { ...TIMELINE_MAP[blk_type[blk]] };
        blk_timeline.sample = {
