// Version 2.1 for pilot SFB ironic errors series of experiments
...
//
// 4 types of trials:
// 1) Reversal noise / Low level
// 2) Reversal noise / High level
// 3) Gaussian noise / Low level
// 4) Gaussian noise / High level
//
...
const PRMS = {
    ...
    noise_reversal_prob: { low: 0.1, high: 0.3 }, // probability of reversing mouse movement
...
class Path {
    ...
    draw_target_path() {
        ...
        for (let x = this.x.length - 1; x > 0; x--) {
            if (this.on_path[x] === 0 || !PRMS.show_error_path) {
...
class Ball {
    ...
    move(dt) {
        ...
        } else if (this.noise_type === "gaussian") {
            let sd = PRMS.noise_gaussian_sd[this.noise_level];
            // If mouse is moved, apply gaussian jitter. Optionally apply it even if not moving?
            // "if I move left it will sometimes go right" -> applies to movement.
            // But gaussian could just be constant jitter? Let's apply constant jitter on every frame.
...
function draw_trial() {
    ...
    if (PRMS.show_condition_info) {
        ...
        p5js.textSize(20);
        p5js.text(`Noise Type: ${BALL.noise_type} | Noise Level: ${BALL.noise_level}`, 20, 30);
    }
...
const TRIAL = {
    ...
    data: {
        stim_type: "ftp",
        noise_type: jsPsych.timelineVariable("noise_type"),
    ...
    questions: [
        {
            ...
            name: "question_1",
            labels: generate_likert_labels(
                PRMS.survey_scale[0],
                PRMS.survey_scale[1],
                PRMS.survey_anchors[0],
                PRMS.survey_anchors[1],
            ),
        },
    ],
};
    ...
    timeline: [
        {
            ...
            conditional_function: function () {
                return PRMS.survey_type === "likert";
            },
        },
        {
            timeline: [SURVEY_SLIDER],
            conditional_function: function () {
                return PRMS.survey_type === "slider";
            },
        },
    ],
};
...
const TRIAL_TABLE = [
    { noise_type: "reversal", noise_level: "low" },
    { noise_type: "reversal", noise_level: "high" },
    { noise_type: "gaussian", noise_level: "low" },
    { noise_type: "gaussian", noise_level: "high" },
];
