...
const task_instructions3 = {
  ...
  canvas_border: canvas_border,
  stimulus:
    "<h3 style='text-align: left;'>Für die Buchstabenaufgabe musst du entscheiden ob der Buchstabe im Alphabet vor I oder nach R kommt.</h3>" +
...
const fixation_cross = {
  ...
  on_start: function (trial) {
    trial.trial_duration =
      prms.fix_duration + getRandomInt(prms.iti[0], prms.iti[1]);
  },
};
...
function drawFeedback() {
  ...
  if (dat.corrCode === 0 || dat.corrCode === 2) {
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      "Wenn der Buchstabe zwischen I und R oder die Zahl zwischen 147",
      0,
      220
    );
    ctx.fillText(
      "und 159 ist, dann erfordert die Aufgabe keine Antwort!",
      0,
      250
    );
  }
...
function codeTrial() {
  ...
  if (
    [dat.corrKey1, dat.corrKey2].includes(dat.key_press) &
    (dat.rt !== null)
  ) {
    corrCode = 1;
  }
  ...
  if (prms.taskMapping[0] === "number") {
    responseTask = prms.respKeys.slice(0, 2).includes(dat.key_press)
      ? "Number"
      : "Letter";
  } else if (prms.taskMapping[0] === "letter") {
    responseTask = prms.respKeys.slice(0, 2).includes(dat.key_press)
...
const save_data = {
  ...
  func: function () {
    let data_filename =
      dirName + "data/version" + version + "/" + expName + "_" + vpNum;
...
const save_interaction_data = {
  ...
  func: function () {
    let data_filename =
      dirName + "interaction_data/" + expName + "_interaction_data_" + vpNum;
