...
/*
 ...
 Task diffuculty is manipulated via contrast of central target item within
 flanker array.
 */
...
function generateFlankerLetters() {
    ...
    for (i = 0; i < letters.length; i++) {
        ...
        for (j = 0; j < letters.length; j++) {
            ...
            letterCombs.push(letters[i] + letters[i] + "<font color='lightgrey'>" + letters[j] + "<font color='black'>" + letters[i] + letters[i]);
        }
    }
    return letterCombs;
}
...
function generate_flankers(letters) {
    ...
    for (var i = 0; i < letters.length; i++) {
        ...
        if (letters[i].length == 5) {
        ...
        } else if (letters[i].length == 49) {
           
            diff = "hard";
            
            if (vowels.includes(letters[i][26])) {
                rd = respDir[0];
                rk = respKeys[0];
            } else {
                rd = respDir[1];
            ...
            if (["A", "E", "O", "U"].includes(letters[i][0]) && ["A", "E", "O", "U"].includes(letters[i][26])) {
                comp = "comp";
            } else if (["B", "H", "T", "S"].includes(letters[i][0]) && ["B", "H", "T", "S"].includes(letters[i][26])) {
                comp = "comp";
            } else {
                comp = "incomp";
            }
...
function codeTrial() {
    ...
    if (data.stim === "flanker_resp") {
        ...
        if (dataP.value !== undefined) {
            if (dataP.letterArray.length > 5){
                dataP.letterArray = dataP.letterArray[0] + dataP.letterArray[1] + dataP.letterArray[26] + dataP.letterArray[47] + dataP.letterArray[48]
            }
            
            if (data.letterArray.length > 5){
                data.letterArray = data.letterArray[0] + data.letterArray[1] + data.letterArray[26] + data.letterArray[47] + data.letterArray[48]
            }
        ...
        if (data.key_press === corrKeyNum && data.rt > tooFast && data.rt < tooSlow) {
            corrCode = 1;
            if (nBlk > 1 && false_feedback_allowed) {  // only give false feedback after practice blocks
...
var writeDataFile = function () {
    var fname = "flanker_pes_2_" + vpNum + ".csv";
...
var alphaNum = {
    ...
    "<h1>ermioni.athanasiadi@student.uni-tuebingen.de</h1>" +
        ...
        for (trl = 0; trl < numberTrials; trl++) {
            ...
            respInt = {
                ...
                data: {
                    ...
                    difficulty: flankers[trl].data.difficulty,
