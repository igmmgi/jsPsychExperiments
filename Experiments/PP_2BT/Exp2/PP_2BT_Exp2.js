...
// prettier-ignore
let resp_text_colours_training = `<span style="color: ${prms.colours[0]}">${ de[prms.colours[0]]}</span> &emsp;&emsp; <span style="color: ${prms.colours[1]}">${ de[prms.colours[1]] }</span><br>
                   (Q-Taste) &emsp;&emsp; (P-Taste) <br>
                   Wenn Farbe <span style="color: ${prms.colours[2]}">${de[prms.colours[2]]}</span> ist kein Taste drücken!<br><br>`;

// response keys for letter task/number task
let resp_text_numbers = `Zahl kleiner 5 &emsp;&emsp;&emsp; Zahl größer 5<br>
                        (Q-Taste) &emsp;&emsp; (P-Taste) <br>`;

let resp_text_letters = `vor "M" &emsp;&emsp;&emsp; nach "M"<br>
                        (Q-Taste) &emsp;&emsp; (P-Taste) <br>`;
...
Du musst auf die Farben eines Quadrates (= Farbaufgabe), Buchstaben (= Buchstabenaufgabe) oder auf Zahlen <br>(= Zahlaufgabe) reagieren. <br><br>
...
function draw_feedback_training() {
  ...
  if ((dat.corrCode === 1) | (dat.corrCode === 4)) {
    ...
    if (dat.colour !== 'black') {
      ...
      ctx.fillStyle = 'black';
      ctx.fillText('Wenn Farbe     ', -140, 50);
      ctx.fillStyle = prms.colours[2];
      ctx.fillText(de[prms.colours[2]], -45, 50);
      ctx.fillStyle = 'black';
      ctx.fillText('keine Taste drücken!', 180, 50);
      ...
      ctx.fillText('Buchstabe vor M        Buchstabe nach M', 0, -15);
      ctx.fillText('(Q-Taste)', -150, 20);
      ctx.fillText('(P-Taste)', 200, 20);
    }
  }
...
// prettier-ignore
const stimuli_background_high_primary = (nVersion < 3) ?
    [
...
// prettier-ignore
const stimuli_background_low_primary = (nVersion < 3) ?
    [
        { response_task: 'background', background_task: 'letter', colour: prms.colours[2], letter_number: "left_letter",  soa: prms.soa[0], corr_key: prms.resp_keys[0], backward_comp: "na"},
        { response_task: 'background', background_task: 'letter', colour: prms.colours[2], letter_number: "right_letter", soa: prms.soa[0], corr_key: prms.resp_keys[1], backward_comp: "na"},
        { response_task: 'background', background_task: 'letter', colour: prms.colours[2], letter_number: "left_letter",  soa: prms.soa[0], corr_key: prms.resp_keys[0], backward_comp: "na"},
