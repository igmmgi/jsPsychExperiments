...
jsPsych.plugins['mouse-box-response'] = (function () {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      down_box: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Down Box',
        default: [0, 0, 100, 100],
        description: 'Down Box',
      },
      left_box_colour: {
        type: jsPsych.plugins.parameterType.STRING,
      ...
      up_box_colour: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Up Box Colour',
        default: 'black',
        description: 'Up Box Colour',
      },
      down_box_colour: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Down Box Colour',
        default: 'black',
        description: 'Down Box Colour',
      },
      box_linewidth: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    let up_responsebox = {
      x: trial.up_box[0] - trial.up_box[2] / 2,
      y: trial.up_box[1] - trial.up_box[3] / 2,
      w: trial.up_box[2],
      h: trial.up_box[3],
    };
    let down_responsebox = {
      x: trial.down_box[0] - trial.right_box[2] / 2,
      y: trial.down_box[1] - trial.right_box[3] / 2,
      w: trial.down_box[2],
      h: trial.down_box[3],
    };
    ...
    function check_response() {
      if (in_box(mpos.x, mpos.y, left_responsebox)) {
        end_loc = 'left';
      } else if (in_box(mpos.x, mpos.y, right_responsebox)) {
        end_loc = 'right';
      } else if (in_box(mpos.x, mpos.y, up_responsebox)) {
        end_loc = 'up';
      } else if (in_box(mpos.x, mpos.y, down_responsebox)) {
        end_loc = 'down';
      }
    }
    ...
    function draw() {
      ...
      if (draw_response_boxes) {
        ...
        ctx.beginPath();
        ctx.lineWidth = trial.box_linewidth;
        ctx.strokeStyle = trial.up_box_colour;
        ctx.rect(up_responsebox.x, up_responsebox.y, up_responsebox.w, up_responsebox.h);
        ctx.stroke();
        // response box down
        ctx.beginPath();
        ctx.lineWidth = trial.box_linewidth;
        ctx.strokeStyle = trial.down_box_colour;
        ctx.rect(down_responsebox.x, down_responsebox.y, down_responsebox.w, down_responsebox.h);
        ctx.stroke();
      }
