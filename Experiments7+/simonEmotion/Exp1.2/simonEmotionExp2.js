// Simon Task:
// VPs respond to the emotion (happy vs. fear) of a face presented to the left/right side of the screen.
...
const PRMS = {
    ...
    respKeys: ['Q', 'P'],
    target: shuffle(['änglichstes', 'glückliches']),
...
const TASK_INSTRUCTIONS2 = {
    ...
    canvas_border: CANVAS_BORDER,
    stimulus: generate_formatted_html({
        text: `In diesem Experiment siehst du glückliche und ängstliche Gesichter.
...
function generateStimulusCombinations(dataset_fear, dataset_happy, dataset_name, image_numbers) {
    ...
    for (let i = 0; i < simon_type.length; i++) {
        ...
        if (simon_type[i] === 'fear_left') {
            ...
            tmp.key = PRMS.respKeys[PRMS.target.indexOf('änglichstes')];
...
// prettier-ignore
const TRIALS_NIMSTIM = generateStimulusCombinations(nimstim_fear, nimstim_happy, 'nimstim', IMAGE_NUMBERS.nimstim);
const TRIALS_MPI_YOUNG = generateStimulusCombinations(mpi_fear_young, mpi_happy_young, 'mpi_young', IMAGE_NUMBERS.mpi_young);
const TRIALS_MPI_MIDDLE = generateStimulusCombinations(mpi_fear_middle, mpi_happy_middle, 'mpi_middle', IMAGE_NUMBERS.mpi_middle);
const TRIALS_MPI_OLD = generateStimulusCombinations(mpi_fear_old, mpi_happy_old, 'mpi_old', IMAGE_NUMBERS.mpi_old);
const TRIALS_KDEF = generateStimulusCombinations(kdef_fear, kdef_happy, 'kdef', IMAGE_NUMBERS.kdef);
const TRIALS_RADBOUD = generateStimulusCombinations(radboud_fear, radboud_happy, 'radboud', IMAGE_NUMBERS.radboud);
const TRIALS_RADIATE1 = generateStimulusCombinations(radiate_fear, radiate_happy, 'radiate', IMAGE_NUMBERS.radiate1);
const TRIALS_RADIATE2 = generateStimulusCombinations(radiate_fear, radiate_happy, 'radiate', IMAGE_NUMBERS.radiate2);
