...
function save_blockwise() {
    ...
    const data_fn = `${DIR_NAME}data/blockwise/${EXP_NAME}_${VP_NUM}`;
    saveData("/Common7+/write_data.php", data_fn, [{ stim: "catch" }, { stim: "vas" }], "csv", [
        "stimulus",
        "trial_type",
        "internal_node_id",
...
const PROLIFIC = {
  ...
  stimulus: generate_formatted_html({
    text:
      `Super, du bist am Ende des Experiments!
       ...
       https://app.prolific.com/submissions/complete?cc=C17S0HTK<br><br>
  ...
  on_finish: function () {
    window.location.replace('https://app.prolific.com/submissions/complete?cc=C17S0HTK');
  },
};
