...
const PRMS = {
    ...
    y_speed_difficulty: { easy: 1, hard: 2 },
};
...
class Path {
    ...
    reset() {
        this.x_path = new Array(CANVAS_SIZE[1] - 100).fill(0);
        this.y_path = range(CANVAS_SIZE[1] - 100, 0, -1);
        this.error = new Array(CANVAS_SIZE[1] - 100).fill(0);
    }

    perlin_noice_coordinates(value) {
        let start = Math.round(Math.random() * 1000);
        for (let i = start; i < CANVAS_SIZE[1] - 100 + start; i++) {
    ...
    calculate_distance(x, y, criterion) {
        ...
        let distance = Math.abs(tmp_x_idx - PATH.x_path[tmp_y_idx - 100]);
...
function draw_trial() {
    ...
    BALL.move();
    PATH.calculate_distance(BALL.x_pos, BALL.y_pos, BALL.diameter); // comment/uncomment to draw green/red correctness feedback
...
function save() {
    ...
    // saveData("/Common/write_data.php", data_fn, { stim_type: "ftp" });
