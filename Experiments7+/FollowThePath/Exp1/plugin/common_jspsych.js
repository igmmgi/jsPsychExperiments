...
function checkVpDemographics() {
    // get age, gender, handedness and VPs consent
    let alert_message = "Bitte beantworten alle Fragen!";
    let id = document.getElementById("prolific_id").value;
    let age = document.getElementById("age").value;
    ...
    if (age !== "" && gender !== "" && hand !== "") {
        return { id: id, age: age, gender: gender, handedness: hand };
    } else {
...
function vpDemographics(form = "demographics.html") {
    return {
        type: jsPsychExternalHtml,
        url: form,
        cont_btn: "start",
        check_fn: function () {
            const vpInfo = checkVpDemographics();
            console.log(vpInfo);
            if (vpInfo !== false) {
                jsPsych.data.addProperties(vpInfo);
                return true;
