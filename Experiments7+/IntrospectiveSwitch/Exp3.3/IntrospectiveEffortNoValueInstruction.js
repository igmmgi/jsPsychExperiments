    ...
    on_finish: function () {
        if (PRMS.count_block >= 12) {
            window.location.assign(
                "https://uni-tuebingen.sona-systems.com/webstudy_credit.aspx?experiment_id=626&credit_token=b61b2c8b6b5a4ba09a129ea6b0016628&survey_code=" +
...
const PRMS = {
    ...
    colour_task_ratio: [17, 83], // should sum to 100!
    ...
    letter_task_ratio: [17, 83], // should sum to 100!
...
const TASK_INSTRUCTIONS4 = {
    ...
    canvas_border: CANVAS_BORDER,
    stimulus: generate_formatted_html({
        text: `Bitte beachte in Blöcken mit freier Wahl:<br><br>
           <span style="font-weight: bold";> Du darfst zwar in jedem Durchgang entscheiden welche der beiden Aufgaben du bearbeiten möchtest,
...
WICHTIG: Versuche in jedem Durchgang zufällig eine der beiden Aufgaben auszuwählen.<br>
...
const BLOCK_FREE_PENALTY_SCREEN = {
    type: jsPsychHtmlKeyboardResponseCanvas,
    canvas_colour: CANVAS_COLOUR,
    canvas_size: CANVAS_SIZE,
    canvas_border: CANVAS_BORDER,
    stimulus: "",
    on_start: function (trial) {
        let dat = jsPsych.data.get().filter({ stim_type: "vtsvas", block: PRMS.count_block, free_forced: "free" });
        let poorPerformance = 0;

        if (dat.count() > 0) {
            let n_trials = dat.count();

            let n_letter = dat.filter({ response_task: "Letter" }).count();
            let n_colour = dat.filter({ response_task: "Colour" }).count();
            let task_bias = Math.max(n_letter, n_colour) / n_trials;

            let n_switch = dat.filter({ repetition_switch: "switch" }).count();
            let n_repeat = dat.filter({ repetition_switch: "repetition" }).count();
            let switch_bias = Math.max(n_switch, n_repeat) / n_trials;

            if (task_bias > 0.9 || switch_bias > 0.9) {
                poorPerformance = 1;
            }
        }
        ...
        if (poorPerformance === 1) {
            trial.stimulus =
                ...
                generate_formatted_html({
                    text: `Du darfst zwar in jeden Durchgang entscheiden welche der beiden Aufgaben du bearbeiten möchtest,
           aber du sollst versuchen beide Aufgaben gleich häufig und in einer zufälligen Reihenfolge zu wählen.</span><br><br>
           Versuche somit die Aufgaben so zu wählen als ob ein zufälliger Münzwurf entscheidet welche
           Aufgabe in einem Durchgang bearbeitet werden soll. <span style="font-weight: bold";>Somit wirst du manchmal die Aufgabe
           wiederholen und manchmal die Aufgabe wechseln</span>. Du sollst aber nicht mitzählen wie häufig du jede
           Aufgabe machst oder vorgeplante Wahlstrategien verwenden. Versuche einfach zufällig eine Aufgabe auszuwählen.<br><br>
           In ${PRMS.waitDur / 1000} Sekunden geht es automatisch weiter...`,
                    align: "left",
                    fontsize: 24,
                    width: "1200px",
                    lineheight: 1.5,
            ...
            trial.stimulus = "";
            trial.trial_duration = 0; // Skip if Performance is good
            trial.response_ends_trial = false;
        }
    },
};
...
function genExpSeq() {
    ...
    for (let blk = 0; blk < PRMS.n_blocks; blk += 1) {
        ...
        if (block_type[blk] === "free") {
            exp.push(BLOCK_FREE_PENALTY_SCREEN);
        }
