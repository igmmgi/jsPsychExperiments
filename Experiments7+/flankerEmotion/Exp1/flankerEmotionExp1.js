...
const prms = {
  ...
  respKeys: ['Q', 'P'],
  target: shuffle(['neutrales', 'positives']),
...
function generateStimulusCombinations(dataset_neutral, dataset_positive, dataset_name, image_numbers) {
  let flanker_type = shuffle(
    repeatArray(['neutral_comp', 'neutral_incomp', 'positive_comp', 'positive_incomp'], image_numbers.length / 4),
  );
  let trials = [];
...
const trials_kdef = generateStimulusCombinations(kdef_neutral, kdef_positive, 'kdef', image_numbers.kdef);
