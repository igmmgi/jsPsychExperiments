...
// Participants perform two type tasks (flanker + simon): letter task (H vs. S) vs. colour task (blue vs. red)
// Overall three "task-type":
// 1) Free-Choice i.e., both tasks available
// 2) Forced-Letter i.e., only letter (flanker) task available
// 3) Forced-Colour i.e., only colour (simon) task available
//
// Responses for each task made with either the index fongers of each hand or middle fingers of each hand
// (Q, W, O, P keys) with task-to-finger mapping randomly selected per participant
//
...
const PRMS = {
    ...
    response_keys_index: ["W", "O"],
    response_keys_middle: ["Q", "P"],
...
if (PRMS.task_side[0] === "Colour") {
    PRMS.response_keys_colour = PRMS.response_keys_index;
    PRMS.response_keys_letter = PRMS.response_keys_middle;
    PRMS.response_stimuli = PRMS.colour_task_colours.concat(PRMS.letter_task_letters);
    PRMS.key_mapping[PRMS.colour_task_colours[0]] = PRMS.response_keys_index[0];
    PRMS.key_mapping[PRMS.colour_task_colours[1]] = PRMS.response_keys_index[1];
    PRMS.key_mapping[PRMS.letter_task_letters[0]] = PRMS.response_keys_middle[0];
    PRMS.key_mapping[PRMS.letter_task_letters[1]] = PRMS.response_keys_middle[1];
} else if (PRMS.task_side[0] === "Letter") {
    PRMS.response_keys_letter = PRMS.response_keys_index;
    PRMS.response_keys_colour = PRMS.response_keys_middle;
    PRMS.response_stimuli = PRMS.letter_task_letters.concat(PRMS.colour_task_colours);
    PRMS.key_mapping[PRMS.letter_task_letters[0]] = PRMS.response_keys_index[0];
    PRMS.key_mapping[PRMS.letter_task_letters[1]] = PRMS.response_keys_index[1];
    PRMS.key_mapping[PRMS.colour_task_colours[0]] = PRMS.response_keys_middle[0];
    PRMS.key_mapping[PRMS.colour_task_colours[1]] = PRMS.response_keys_middle[1];
}
// PRMS.response_keys = PRMS.response_keys_index.concat(PRMS.response_keys_middle);
console.log(PRMS.key_mapping);
...
const TASK_INSTRUCTIONS_TEXT = generate_formatted_html({
    text: `In diesem Experiment gibt es <span style="font-weight: bold;">zwei</span> Aufgaben. Jede Aufgabe wird mit dem Zeige- oder dem Mittelfinger bearbeitet.<br><br>
<span style="font-weight: bold;">${EN_DE[PRMS.task_side[0]]}</span> = Zeigefinger: Bitte platziere hierzu den Zeigefinger auf die Tasten <span style="font-weight: bold;">"${PRMS.response_keys_index[0]}"</span> und <span style="font-weight:bold;">"${PRMS.response_keys_index[1]}"</span>.<br><br>
<span style="font-weight: bold;">${EN_DE[PRMS.task_side[1]]}</span> = Mittlefinger: Bitte platziere hierzu den Mittelfinger auf die Tasten <span style="font-weight: bold;">"${PRMS.response_keys_middle[0]}"</span> und <span style="font-weight:bold;">"${PRMS.response_keys_middle[1]}"</span>.<br><br>
Drücke eine beliebige Taste, um fortzufahren.`,
    align: "left",
    fontsize: 30,
    width: "1200px",
...
const RESPONSE_MAPPING = generate_formatted_html({
    //text: `${EN_DE[PRMS.task_side[0]]} = Zeigefinger${"&emsp;".repeat(6)} ${EN_DE[PRMS.task_side[1]]} = Mittelfinger<br><br>
    text: `${EN_DE[PRMS.task_side[0]]} = Zeigefinger${"&emsp;".repeat(2)}<br>${EN_DE[PRMS.task_side[1]]} = Mittelfinger<br><br>
${EN_DE[PRMS.response_stimuli[2]]} ${pad_me("vs.", 28)} ${EN_DE[PRMS.response_stimuli[0]]}${"&emsp;".repeat(12)}${EN_DE[PRMS.response_stimuli[1]]} ${pad_me("vs.", 25)} ${EN_DE[PRMS.response_stimuli[3]]}<br>
...
Versuche so schnell wie möglich zu sein ohne zu viele Fehler zu machen!<br>`,
                align: "left",
                fontsize: 30,
                width: "1200px",
                lineheight: 1.25,
            ...
            trial.func_args = [
                {
                    ...
                    draw_letters: false,
                    letters: letters,
                    colour_comp: trial.data.colour_task_comp,
                    colour_key: trial.data.colour_task_key,
                },
                {
                    free_forced: trial.data.free_forced,
                    draw_dots: true,
